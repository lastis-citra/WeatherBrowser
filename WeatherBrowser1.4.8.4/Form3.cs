...
namespace WeatherBrowser
{
    public partial class Form3 : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            string url = "http://weather.yahoo.co.jp/weather/";
            ...
            foreach (HtmlNode node in nodeList)
            {
                ...
                try
                {
                    if (Regex.IsMatch(node["href"], @"^http://weather.yahoo.co.jp/weather/jp/\d[\dabcd]*/"))
                    {
            ...
            foreach (string url2 in url_array1)
            {
                ...
                foreach (HtmlNode node in nodeList)
                {
                    ...
                    try
                    {
                        if (Regex.IsMatch(node["href"], @"^http://weather.yahoo.co.jp/weather/jp/\d[\dabcd]*/\d+"))
                        {
            ...
            foreach (List<string> u_array in url_array2)
            {
                ...
                foreach (string url2 in u_array)
                {
                    ...
                    foreach (HtmlNode node in nodeList)
                    {
                        ...
                        try
                        {
                            if (Regex.IsMatch(node["href"], @"^http://weather.yahoo.co.jp/weather/jp/\d[\dabcd]*/\d+/\d+"))
                            {
                                tmp_url_array.Add(node["href"]);

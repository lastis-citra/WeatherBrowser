...
namespace WeatherBrowser
{
    public partial class Form3 : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            // ------------------------------------------------------
            //ダウンロードするURL（警報・注意報）
            string url2 = "http://tenki.jp/warn/";
            ...
            foreach (HtmlNode node in nodeList)
            {
                ...
                try
                {
                    if (Regex.IsMatch(node["href"], @"^/warn/pref-[0-9]+.html") &&
                        !Regex.IsMatch(node.InnerText, @"^<img>"))
                    {
                        //MessageBox.Show(node["href"]);
                        l_list_warn.Add(node.InnerText, node["href"]);
                    }
                }
                catch (Exception e2)
                {
            ...
            {
                sw.Write(keyValuePair.Key + "," + keyValuePair.Value + "\r\n");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            foreach(string key in name_url_2_2.Keys){
                ...
                {
                    Form1.url_warn = name_url_2_2[key];
                    break;
                }
            }

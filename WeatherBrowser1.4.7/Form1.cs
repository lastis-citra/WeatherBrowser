...
namespace WeatherBrowser
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string url8 = "http://tenki.jp/forecast/pref-" + tmp_url_abst[6] + ".html";
            ...
            try
            {
                ...
                //string filePath = @"./cache/forecast_" + st_abst2[0] + ".cache";
            ...
            foreach (HtmlNode node in nodeList)
            {
                ...
                else if (date_flag3)
                {
                    ...
                    //1.4.6 土日の曜日がうまく表示されなくなっていたので修正
                    //string[] st1 = node.InnerText.Split('<');
                    //string[] st2 = node.InnerText.Split('>');
            ...
            // v1.4.6 指数情報が1ページにまとめて掲載されるようになったため
            //i = 0;
            ...
            foreach (HtmlNode node in nodeList)
            {
                ...
                if (node.Attributes.Keys.Count > 0 && node.Attributes.ContainsKey("class") && node.Attributes["class"] == "is_normal" && node.InnerText.Contains("発表なし"))
                {
                    // 発表なしなら何もしない
                    break;
                }
                if (node.Attributes.Keys.Count > 0 && node.Attributes.ContainsKey("class") && node.Attributes["class"] == "is_warn")
                {
                    string tmp_warn = node.InnerText.Replace("警報", "");
                    warning_array = ChangeStatusWarnNotice(tmp_warn, warning_array);
                }
                if (node.Attributes.Keys.Count > 0 && node.Attributes.ContainsKey("class") && node.Attributes["class"] == "is_alert")
                {
            ...
            ////----------------------------------------------------------
            // 天気概況
            nodeList = doc8.GetNodesByTagName("p");

            //foreach (HtmlNode node in nodeList)
            //{
            //    if (node.Attributes.ContainsKey("class") && node.Attributes["class"] == "sub_title")
            //    {
            ...
            //    }
            //}
            textBox1.Text = nodeList[3].InnerText.Replace("<br />", "\r\n").Replace("<br>", "\r\n");
            ...
            //    }
            //}
            label8.Text = nodeList[2].InnerText;
            ...
            // v1.4.6　レイアウト変更によりここはとりあえず不要になったようなので一度消しておく
            ...
            for (int j = 0; j < 2; j++)
            {
                for (int k = 0; k < 9; k++)
                {
                    sw4.Write(notice_array[j, k] + "," + warning_array[j, k] + "\n");
